...
def _resolve_system_prompt(body: AgentIn) -> str:
    ...
    if outbound:
        return outbound
    return f"You are {body.name}, a Trango Tech phone agent."
...
async def create_agent(body: AgentIn, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    taken = {
        row[0]
        for row in (await db.execute(select(Agent.slug))).all()
    }
    ...
    system_prompt = _resolve_system_prompt(body)
    agent = Agent(
        name=body.name,
        slug=slug,
        type=body.type,
        did=(body.did or "").strip() or None,
...
async def update_agent(agent_id: int, body: AgentIn, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    a.name = body.name
    a.type = body.type
    a.did = (body.did or "").strip() or None
