...
const CRM_TOOLS = ['create_lead', 'search_contacts', 'create_note', 'update_lead_status', 'transfer_to_support'];
...
const TYPES = [
  ...
  { value: 'support', label: 'FAQ / Support' },
...
export default function Agents() {
  ...
  return (
    <div className="p-6 lg:p-10">
      <PageHeader
        title="AI Agents"
        subtitle="Inbound callbacks route to sales · Support transfers mid-call · Extensions auto-assigned"
          ...
          <GlassCard key={a.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {a.type === 'support' && <Badge variant="default">FAQ</Badge>}
                {a.type === 'outbound_sales' && <Badge variant="warn">Outbound</Badge>}
                {a.inbound_extension && (
              ...
              {a.type === 'outbound_sales' ? (
                ...
                <p className="text-xs text-violet-400/80 mt-2 flex items-center gap-1">
                  <Phone className="w-3 h-3" /> Shared inbound DID / ext 700 (callback routing)
                </p>
              ) : null}
            </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-lg bg-[#0c0c12] border-l border-white/10 h-full overflow-auto p-6 space-y-4">
            ...
            <Field label="DID (optional — shared company DID if blank)">
            ...
            <Field label="Inbound prompt">
              <textarea rows={8} className={cn(inputCls, 'resize-y')} placeholder={isSupport ? 'FAQ/support persona and call flow...' : 'Sales inbound persona and 9-stage funnel...'} value={form.inbound_prompt_template} onChange={e => setForm(f => ({ ...f, inbound_prompt_template: e.target.value }))} />
            </Field>
            {!isSupport && (
            ...
            {!editing && (
              <p className="text-xs text-zinc-500">SIP extension is assigned automatically when the agent is created.</p>
            )}
            <Field label="Tools">
              <div className="space-y-2 text-sm text-zinc-400">
                {(isSupport ? [...RAG_TOOLS, 'end_call'] : [...CRM_TOOLS, ...RAG_TOOLS, ...SEARCH_TOOLS]).map(t => (
